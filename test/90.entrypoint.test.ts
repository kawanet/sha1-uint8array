...
import type * as declared from "sha1-uint8array"
import * as m from "../lib/sha1-uint8array.ts"
...
test("require entry (.cjs)", () => {
    const m = require("sha1-uint8array")
    assert.equal(typeof m.createHash, "function")
})

test("minified entry (.min.js)", () => {
    const cjs = require.resolve("sha1-uint8array")
